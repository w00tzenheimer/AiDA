...
def setup_env():
    ...
    if not ida_sdk_dir.exists():
        ...
        run_command(
            f"git clone --depth 1 https://github.com/HexRaysSA/ida-sdk.git {ida_sdk_dir}"
        )
    else:
    ...
    if not ida_cmake_dir.exists():
        ...
        run_command(
            f"git clone --depth 1 https://github.com/allthingsida/ida-cmake.git {ida_cmake_dir}"
        )
...
def copy_artifact(extension, search_dir=None):
    ...
    if not found:
        print(
            f"Error: Could not find {filename} in any of: {[str(d) for d in search_dirs]}"
        )
        sys.exit(1)
...
def main():
    ...
    copy_parser.add_argument(
        "--extension", required=True, help="Plugin extension (dll, so, dylib)"
    )
    copy_parser.add_argument(
        "--search-dir", help="Explicit directory to search for artifact"
    )
